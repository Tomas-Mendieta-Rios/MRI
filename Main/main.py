...
class StreamLit:
    ...
    def sidebar(self):
        ...
        st.sidebar.checkbox("All Dates", value=True, key='all_dates_checkbox')
        ...
        st.sidebar.checkbox("All Ages", value=True, key='all_ages_checkbox')
        ...
        st.sidebar.checkbox("All Genders", value=True, key='all_genders_checkbox')
        ...
        st.sidebar.checkbox("All Recommendations", value=True, key='all_recommendations_checkbox')
        ...
        st.sidebar.checkbox("Categorize Ages", value=False, key='categorize_ages_checkbox')
        if st.session_state['categorize_ages_checkbox']:
            st.sidebar.subheader("Define Age Categories")
            st.sidebar.slider("Min Age for Jóvenes", min_value=0, max_value=100, value=15, key='age_joven_min_slider')
    ...
    # In the initialize_filters function within StreamLit class
...
class Filters:
    ...
    def dates(self):
        ...
        self.result = self.result[(self.result['date_recepcion_data'] >= date_min) & (self.result['date_recepcion_data'] <= date_max)] 
...
def main():
    ...
    df_filtered = filters.choose_filter()  # Apply the filters here directly
    
      # Check for the correct session state key
    df_filtered = filters.result  # Get the filtered result
    
    
    # Show the number of users after applying filters
    st.write(f'Cantidad de usuarios: {len(df_filtered)}')  
    st.write('Data filtrada', df_filtered)
